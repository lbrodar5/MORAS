class Parser:
    ...
    def _push(self, src, loc, n):
        if(not loc.isdigit()):
            self._flag = False
            Parser._error("Push", n, "Undefined location \"" + loc + "\".");
            return ""
    ...
    def _pop(self, dst, loc, n):
        if(not loc.isdigit()):
            self._flag = False
            Parser._error("Pop", n, "Undefined location \"" + loc + "\".");
            return ""
    ...
    def _checkLabels(self):
        for label in self._jmplabels:
            if(label[0] not in self._labels):
                self._flag = False
                Parser._error("Jmp", label[1], "Undefined label "+ label[0]);
                return
        return
